...
class SpendingManagerApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: AuthGate(), // Đảm bảo đây là AuthGate nếu bạn đang sử dụng class AuthGate
    );
  }
}

class AuthGate extends StatelessWidget { // Đảm bảo tên class là AuthGate
  @override
  Widget build(BuildContext context) {
    return StreamBuilder<User?>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return LoginScreen(); // Trang đăng nhập nếu chưa có người dùng
        }
        // Nếu đã đăng nhập, chuyển đến trang chủ
        return TransactionListScreen(); // Trang chủ sau khi đăng nhập
      },
    );
  }
}
